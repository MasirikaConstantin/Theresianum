...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Salles', href: '/espaces' },
    { title: 'Modifier l\'espace', href: '#' },
];

export default function SalleEdit({ auth, vocations,espace,flash }: { auth: Auth; vocations: string[],espace :Salle,flash: { error: string; success: string } }) {
    const { data, setData, errors, put, processing } = useForm({
        nom: espace.nom,
        capacite_max: espace.capacite_max,
        vocation: espace.vocation,
        prix_journee: espace.prix_journee,
        prix_nuit: espace.prix_nuit,
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('espaces.update', espace.id));
    };
    ...
    return (
        <AppLayout auth={auth} breadcrumbs={breadcrumbs}>
            <Head title="Modifier l'espace" />
            <div className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-6">
                <div className="flex items-center gap-4">
                    <Link href={route('espaces.show', espace.ref)}>
                        <Button variant="outline" size="sm">
                            <ArrowLeft className="h-4 w-4 mr-2" />
                            Retour
                        </Button>
                    </Link>
                    <h1 className="text-2xl font-bold tracking-tight">Modifier l'espace</h1>
                </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-2 md:col-span-2">
                                <Label htmlFor="nom">Nom de la espace *</Label>
                                <Input
                                    id="nom"
                                    value={data.nom}
                        ...
                        <div className="flex gap-4">
                            <Button type="submit" disabled={processing}>
                                {processing ? 'Modification...' : 'Modifier l\'espace'}
                            </Button>
                            <Link href={route('espaces.show', espace.ref)}>
