...
import { Pencil, Plus, Trash2, Eye, Search, Calendar } from 'lucide-react';
...
export default function ReservationIndex({ auth, reservations, statuts }: { 
    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            router.get(route('espaces-reservations.index'), { 
                search, 
                statut: statutFilter
            }, {
                preserveState: true,
    ...
    const handleDelete = (reservation: Reservation) => {
        router.delete(route('espaces-reservations.destroy', reservation.id));
    };

    const handleStatusChange = (reservationId: number, newStatut: string) => {
        router.patch(route('espaces-reservations.update-status', reservationId), { statut: newStatut });
    };
            ...
            <div className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-6">
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        <h1 className="text-2xl font-bold tracking-tight">Réservations des espaces</h1>
                    </div>
                            ...
                            {reservations.data.map((reservation: Reservation, index: number) => {
                                ...
                                return (
                                    <TableRow key={reservation.id}>
                                        ...
                                        <TableCell>
                                            <div className="space-y-1">
                                                <div className="text-sm">
                                                    <strong>Début:</strong> {debut.date} à {debut.time}
                                                </div>
                                                <div className="text-sm">
                                                    <strong>Fin:</strong> {fin.date} à {fin.time}
                                                </div>
                                            </div>
                                        </TableCell>
                                        <TableCell>
                                        ...
                                        <TableCell>
                                            <div className="flex gap-2">
                                                <Link href={route('espaces-reservations.show', reservation.ref)}>
                                                    <Button variant="outline" size="sm">
                                                        <Eye className="h-4 w-4" />
                                                    </Button>
                                                </Link>
                                                {canUpdate && (
                                                    <Link href={route('espaces-reservations.edit', reservation.ref)}>
                                                        <Button variant="outline" size="sm">
