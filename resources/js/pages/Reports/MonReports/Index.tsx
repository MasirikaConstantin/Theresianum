...
export default function Reports({ auth, ventes, depenses, filters }: ReportProps) {
  ...
  const generatePdfUrlSyntese = () => {
    const params = new URLSearchParams()
    params.append('start_date', filters.start_date)
    params.append('end_date', filters.end_date)
    if (filters.user_id) params.append('user_id', filters.user_id.toString())
    return `/reports/pdf-synthese?${params.toString()}`
  }

  const breadcrumbs : BreadcrumbItem[]  = [
    { title: 'Tableau de bord', href: route('dashboard') },
    { title: 'Mon Rapports', href: route('reports.index') },
    {title : `Rapports de vente `, href : '#'}
  ]
  return (
    <AppLayout
      auth={auth}
    ...
    >
      ...
      <div className="space-y-6 p-6">
        ...
        <div className="flex items-center justify-between mx-4">
          ...
          <Button asChild>
            <a href={generatePdfUrlSyntese()} target="_blank">
              Exporter la syntèse PDF
            </a>
          </Button>
        </div>
        </Card>
                    ...
                    <Calendar
                      ...
                      onSelect={(date) => {
                        if (date) {
                          window.location.search = new URLSearchParams({
                            ...filters,
                            end_date: format(date, 'yyyy-MM-dd','PPP',{ locale: fr }),
                          }).toString()
                        }
                      }}
                      initialFocus
                ...
                <TableBody>
                  {depenses.map((depense) => (
                    <TableRow key={depense.id}>
                      <TableCell>{format(new Date(depense.created_at), 'dd/MM/yyyy HH:mm')}</TableCell>
