...
export default function SalleIndex({ auth, salles, vocations }: { 
    auth: Auth;
    salles: any;
    vocations: string[];
}) {
    ...
    const handleStatusChange = (salleId: number, isAvailable: boolean) => {
        router.patch(route('salles.update-status', salleId), { disponible: isAvailable });
    };
            ...
            <div className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-6">
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        <h1 className="text-2xl font-bold tracking-tight">Gestion des salles</h1>
                    </div>
                ...
                <div className="rounded-lg border shadow-sm">
                    <Table>
                        <TableCaption>
                            {salles.total > 0 ? (
                                `Affichage des salles ${salles.from} à ${salles.to} sur ${salles.total}`
                            ) : (
                        ...
                        <TableBody>
                            {salles.data.map((salle: Salle, index: number) => (
                                        ...
                                        <div className="flex items-center gap-2">
                                            {canUpdateStatus && (
                                                <Switch
                                                    ...
                                                    onCheckedChange={(checked) => handleStatusChange(salle.id, checked)}
                                                />
                                            )}
                            ...
                            <PaginationContent>
                                <PaginationItem>
                                    <PaginationPrevious 
                                        ...
                                        onClick={(e) => {
                                            if (!salles.prev_page_url) {
                                                e.preventDefault();
                                                return;
                                            }
                                            e.preventDefault();
                                            router.get(salles.prev_page_url, {}, {
                                                preserveState: true,
                                                replace: true,
                                                preserveScroll: true
                                            });
                                        }}
                                        className={!salles.prev_page_url ? 'pointer-events-none opacity-50' : ''}
                                    />
                                </PaginationItem>
                                
                                {salles.links.slice(1, -1).map((link: any, index: number) => (
                                ...
                                <PaginationItem>
                                    <PaginationNext 
                                        ...
                                        onClick={(e) => {
                                            if (!salles.next_page_url) {
                                                e.preventDefault();
                                                return;
                                            }
                                            e.preventDefault();
                                            router.get(salles.next_page_url, {}, {
                                                preserveState: true,
                                                replace: true,
                                                preserveScroll: true
                                            });
                                        }}
                                        className={!salles.next_page_url ? 'pointer-events-none opacity-50' : ''}
                                    />
                                </PaginationItem>
                            </PaginationContent>
                        </Pagination>
